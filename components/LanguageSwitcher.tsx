...
function LanguageSwitcher() {
  ...
  return (
    <label className='flex items-center relative w-max cursor-pointer select-none'>
      <input type='checkbox' onClick={handleChange} defaultChecked={isChecked} className={`${styles.input} appearance-none transition-colors cursor-pointer w-14 h-7 rounded-full bg-blue-500 checked:bg-gray-400`} />
      ...
      <span className={`${styles.span} w-7 h-7 right-7 absolute rounded-full transform transition-transform bg-gray-200`} />
    </label>
  );
}
